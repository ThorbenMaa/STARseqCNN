...
Description:        Script loads STARseq data provided by the Kaikkonen lab. One-hot-encoded sequences of all possible SNVs of the input sequences are generated and the corresponding predictions form a
                    CNN trained using the "train_or_eval_CNNs_spec.py" script are calculated. The approach is called ISM by Agrawal and Kelley, 2022 (PMID: 36419176). 
...
Inputs:             Input 1 are labels also used for model training and evaluation. Input 2 are sequences also used for model training and evaluation. Both files are required to make sure that only sequences
                    ...
                    Input 5 is a string attached to the names of the outputfiles.
...
example command:    python ism_TM.py 2023-01-10_22-29-33\ myCounts.minDNAfilt.depthNorm.keepHaps\ -\ starr.haplotypes.oligo1.txt starrseq-all-final-toorder_oligocomposition.csv CNN_StarSeq_model_Minna_deepSTAR_lr0.001 2023-01-10_22-29-33\ myCounts.minDNAfilt.depthNorm.keepHaps\ -\ starr.haplotypes.oligo2.txt HEPG2_untreated_pilot_vs_TeloHEAC_CTRL
...
"""
print("initialize arrays with hot-hot-encoded sequences as booleans for tfmodisco")
seqs_tfmodisco_format=np.zeros([numberOfSeqs, seq_length, 4])
for i in tqdm(range (0, numberOfSeqs, 1)):
...
hypothetical_contribution_scores_mean_diff=np.zeros([numberOfSeqs, seq_length, 4])

print("fill arrays with hypothetical importance scores for tfmodisco")
for i in tqdm(range (0, numberOfSeqs, 1)):#iterate over numberOfSeqs
    ...
    for j in range (0, seq_length, 1):
        for k in range (0, 4, 1):
            hypothetical_contribution_scores_mean_diff[ i, j ,k] = predictions_alt[j*4+k,0]




#print shapes (number of Seqs x seq length x 4)
#print(hypothetical_contribution_scores_mean_diff.shape)
#print(seqs_tfmodisco_format.shape)
print("data saved to "+str(os.getcwd()))
#save contribution scores as npz files
np.savez(str(os.getcwd())+'/hypothetical_contribution_scores_mean_diff'+str(sys.argv[5])+'.npz', hypothetical_contribution_scores_mean_diff)


#save sequences as npz file
#np.savez(str(os.getcwd())+'/Sequences_diff'+str(sys.argv[5])+'.npz', seqs_tfmodisco_format)
