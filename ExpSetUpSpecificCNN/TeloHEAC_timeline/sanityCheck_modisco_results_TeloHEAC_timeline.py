"""
Description:            This scripts plots Boxplots without outlyers of experimental STARseq activity for Sequences containing a motif of interest for all 3 TeloHEAC set ups. Note that the motifs should be provided by the user
...
Output:                  Boxplots for activities of motifs of interest across TEloHEAC set ups

example bash command:   python ./ExpSetUpSpecificCNN//TeloHEAC_timeline/sanityCheck_modisco_results_TeloHEAC_timeline.py 2023-01-10_22-29-33\ myCounts.minDNAfilt.depthNorm.keepHaps\ -\ starr.haplotypes.oligo1.txt starrseq-all-final-toorder_oligocomposition.csv 2023-01-10_22-29-33\ myCounts.minDNAfilt.depthNorm.keepHaps\ -\ starr.haplotypes.oligo2.txt
...
#define cell type
cellType=[ "TeloHAEC_CTRL", "TeloHAEC_IL1b_6h", "TeloHAEC_IL1b_24h"]#"cell_3T3_diff_CTRL", "ccell_3T3_undiff_CTRL", "cell_3T3_undiff_TGFB", "RAW_CTRL" , "RAW_IL1B", "RAW_TGFB", "TeloHAEC_CTRL", "TeloHAEC_IL1b_24h", "TeloHAEC_IL1b_6h", "HASMC_untreatedPilot", "HASMC_Chol", "HepG2_untreatedPilot"]
...
for i in range (0, len(motivsToCheck), 1):
    ...
    for j in range (0, len(cellType), 1):
        ...
        df_IDs_seqs_reg_labels_hasMotiv_diff = (df_IDs_seqs_reg_labels.loc[(df_IDs_seqs_reg_labels['enhancer'].str.contains(str(motivsToCheck[i])))==True]["mean_"+str(cellType[0])].to_numpy()
                                                -df_IDs_seqs_reg_labels.loc[(df_IDs_seqs_reg_labels['enhancer'].str.contains(str(motivsToCheck[i])))==True]["mean_"+str(cellType[j])].to_numpy()
                                                )
        data.append(df_IDs_seqs_reg_labels_hasMotiv_diff)
        xTicks.append(str(cellType[0])+" - "+str(cellType[j])+" Difference +")
        ...
        df_IDs_seqs_reg_labels_hasNotMotiv_diff = (df_IDs_seqs_reg_labels.loc[(df_IDs_seqs_reg_labels['enhancer'].str.contains(str(motivsToCheck[i])))==False]["mean_"+str(cellType[0])].to_numpy()
                                                -df_IDs_seqs_reg_labels.loc[(df_IDs_seqs_reg_labels['enhancer'].str.contains(str(motivsToCheck[i])))==False]["mean_"+str(cellType[j])].to_numpy()
                                                )
    ...
    plt.tight_layout()       
    plt.savefig("boxplot_timeline_"+str(motivsToCheck[i])+".svg")
    plt.close()
