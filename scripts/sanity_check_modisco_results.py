...
Input:                  Input 1 are labels (experimental activities). Input 2 are sequences. Note that the motif of interest has to be provided using the "motivsToCheck" array in this file and are not parsed together with the
                        bash command! Input 3 is the cell type or expeimental set-up used in the Manuscript. Options are : cell_3T3_diff_CTRL, mean_ccell_3T3_undiff_CTRL, mean_cell_3T3_undiff_TGFB, mean_TeloHAEC_IL1b_24h, ...
...
for i in range (0, len(motivsToCheck), 1):
    ...
    df_IDs_seqs_reg_labels_hasMotiv=df_IDs_seqs_reg_labels.loc[(df_IDs_seqs_reg_labels['enhancer'].str.contains(str(motivsToCheck[i])))==True]["mean_"+str(cellType)].to_numpy()
    df_IDs_seqs_reg_labels_hasntMotiv=df_IDs_seqs_reg_labels.loc[(df_IDs_seqs_reg_labels['enhancer'].str.contains(str(motivsToCheck[i])))==False]["mean_"+str(cellType)].to_numpy()
    data=[df_IDs_seqs_reg_labels_hasMotiv, df_IDs_seqs_reg_labels_hasntMotiv]
    print(data)
    plt.boxplot(data, showfliers=False)
    plt.set_ylim=(-2, 100)
    plt.ylabel("experimental activity")
    plt.xticks([1, 2], [str(motivsToCheck[i])+" n="+str(data[0].shape), 'other'+" n="+str(data[1].shape)])
    plt.savefig("boxplot_"+str(cellType)+str(motivsToCheck[i])+".svg")
    plt.close()
